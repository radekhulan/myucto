...
ALTER TABLE payroll_enforcement_xmlzam_responses
  ADD COLUMN IF NOT EXISTS includes_wages TINYINT(1) NOT NULL DEFAULT 1 AFTER response_identifier;

ALTER TABLE payroll_enforcement_xmlzam_responses
  DROP CONSTRAINT IF EXISTS chk_xmlzam_response_includes_wages;

ALTER TABLE payroll_enforcement_xmlzam_responses
  ADD CONSTRAINT chk_xmlzam_response_includes_wages CHECK (includes_wages IN (0, 1));
...
BEGIN
  ...
  IF (NEW.includes_wages = 0 AND manifest_rows <> 0)
     OR (NEW.includes_wages = 1 AND (manifest_rows < 1 OR manifest_rows <> approved_rows)) THEN
    SIGNAL SQLSTATE '45000'

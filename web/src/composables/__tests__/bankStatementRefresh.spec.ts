...
function setup() {
  const start = source.indexOf('async function refreshTransactions()')
  const end = source.indexOf('\nonMounted(', start)
  const code = ts.transpileModule(source.slice(start, end), { compilerOptions: { target: ts.ScriptTarget.ES2022 } }).outputText
  const context = {
    route: { params: { id: '7' } }, statusFilter: ref('unmatched'), postingFilter: ref(''),
    txPage: ref(3), txTotal: ref(120), txPages: ref(3), loading: ref(false),
  ...
  const refresh = runInNewContext(`${code}\nrefreshTransactions`, context) as () => Promise<void>

...
describe('PayrollOperationalHealthPanel', () => {
  beforeEach(() => {
    m.operationalHealth.mockResolvedValue({
      document_batches: { queued: 1, running: 2, retry_wait: 3, failed: 4 },
      period_export_jobs: { queued: 12, processing: 13, retry_wait: 14, failed: 15 },
  ...
  it('uses a success tone for fully settled liabilities', async () => {
    m.operationalHealth.mockResolvedValue({
      document_batches: { queued: 0, running: 0, retry_wait: 0, failed: 0 },
      period_export_jobs: { queued: 0, processing: 0, retry_wait: 0, failed: 0 },

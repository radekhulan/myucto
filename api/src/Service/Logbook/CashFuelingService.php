...
final class CashFuelingService
{
    ...
    /**
     * Pokladní doklady, které vypadají jako tankování.
     *
     * @param array{year?:int, only_unscanned?:bool} $filters
    ...
    public function candidates(int $supplierId, array $filters = []): array
    {
        ...
        $params[] = FuelKeywords::SQL_REGEXP;
        ...
        foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $r) {
            ...
            if ($scanned && !empty($filters['only_unscanned'])) {
                continue;
            }
            $out[] = [
                'id'              => (int) $r['id'],
    ...
    public function autoFromCashDocument(int $supplierId, int $cashDocumentId, ?int $userId): ?array
    {
        ...
        foreach ($this->candidates($supplierId) as $c) {
            if ($c['id'] === $cashDocumentId) {
                return $c['scanned'] ? null : $this->scan($supplierId, $cashDocumentId, null, $userId);
            }
        }
        return null;
    }

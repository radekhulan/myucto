...
final class OpeningStockImportService
{
    public const STAGE_KIND = 'stock_opening_import_stage';
    public const APPLY_KIND = 'stock_opening_import_apply';
    ...
    public function preview(int $supplierId, int $sourceId, array $config, ?int $createdBy): array
    {
        ...
        $profile = OpeningStockImportProfile::normalize($config);
        $id = $this->jobs->enqueue($supplierId, self::STAGE_KIND, [
            'source_id' => $sourceId,
            'source_sha256' => $source['sha256'],
            'profile' => $profile,
        ], CatalogImportReader::MAX_ROWS, createdBy: $createdBy);
    ...
    public function apply(int $supplierId, int $previewId, ?int $createdBy): array
    {
        ...
        if ($pdo->inTransaction()) {
            throw new \LogicException('Opening stock apply requires its own transaction.');
        }
        ...
        try {
            ...
            if ($source === null || $source['kind'] !== self::STAGE_KIND) {
                throw new EshopException('not_found', 'Náhled počátečních zásob nenalezen.', 404);
            }
            if ($source['status'] !== 'completed' || (($source['report']['counts']['failed'] ?? 0) + ($source['report']['counts']['conflict'] ?? 0)) > 0) {
                throw new EshopException('job_state_conflict', 'Náhled obsahuje chyby nebo ještě není dokončen.', 409);
            }

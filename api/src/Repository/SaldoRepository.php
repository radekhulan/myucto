...
final class SaldoRepository
{
     ...
     * vůbec dostala k filtru. Definitivní rozhodnutí o otevřenosti dělá pořád
     * `SaldoService::buildAccount()`; SQL filtr je jeho NADMNOŽINA, takže výsledek
     * sestavy je shodný s dobou, kdy se filtrovalo výhradně v PHP.
     *
    ...
    public function openItems(
        ...
        if ($accountCode === '324') {
            return $this->fetchReceivedAdvances($supplierId, $accountId, $asOf, $limit, $partnerId);
        }
        if ($accountCode === '314') {
            return $this->fetchPaidAdvances($supplierId, $accountId, $asOf, $limit, $partnerId);
        }
        return array_merge(
            $this->fetchOpenInvoices($supplierId, $accountId, $asOf, $limit, $partnerId),
            $this->fetchOpenPurchases($supplierId, $accountId, $asOf, $limit, $partnerId),
        );
    }
    ...
    /** `LIMIT n` (int, nikdy ne uživatelský vstup) nebo prázdný řetězec. */
    private static function limitSql(?int $limit): string
    {
        return $limit === null ? '' : ' LIMIT ' . max(1, $limit);
    }
     ...
     */
    private function fetchReceivedAdvances(int $supplierId, int $accountId, string $asOf, ?int $limit, ?int $partnerId = null): array
    {
        $sql =
            "WITH params AS (
            ...
            SELECT p.id AS doc_id,
             ...
             ORDER BY cl.company_name, p.due_date, p.id" . self::limitSql($limit);
     ...
     * @return list<array<string,mixed>>
     */
    private function fetchPaidAdvances(int $supplierId, int $accountId, string $asOf, ?int $limit, ?int $partnerId = null): array
    {
        $sql =
            "WITH params AS (
                SELECT CAST(? AS UNSIGNED) AS supplier_id,
     ...
     * @return list<array<string,mixed>>
     */
    private function fetchOpenInvoices(int $supplierId, int $accountId, string $asOf, ?int $limit, ?int $partnerId = null): array
    {
        ...
        $sql =
            ...
            HAVING " . self::openFilterSql($bookedExpr, $ratio) . "
             ORDER BY cl.company_name, d.due_date, d.id" . self::limitSql($limit);
        $stmt = $this->db->pdo()->prepare($sql);
        $stmt->execute(self::asOfParams($sql, $asOf));
     ...
     * @return list<array<string,mixed>>
     */
    private function fetchOpenPurchases(int $supplierId, int $accountId, string $asOf, ?int $limit, ?int $partnerId = null): array
    {
